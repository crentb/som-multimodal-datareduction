"""
SOM visualization + k-means clustering engine (MODIFIED).

This module is a MODIFIED version of the visualization layer of ``tfprop_sompy``
("Self-Organizing Map Data-Mining for Thermo-Fluid Properties"), originally
developed by Gota Kikugawa and Yuta Nishimura (Tohoku University), which itself
builds on the SOMPY library by Vahid Moosavi (sevamoo) and collaborators. Both
upstreams are Apache-2.0; see ``ACKNOWLEDGMENTS.md`` and the repository ``NOTICE``.

Modifications (c) 2026 Cameron B. Renteria (Apache-2.0):
  * decoupled from the global ``tfprop_config`` module -- tunable parameters are
    now passed as arguments / module constants instead of read from a shared
    global, so the engine is import-safe and reusable;
  * repointed every SOM import at the vendored, namespaced SOMPY copy
    (``som_multimodal._vendor.sompy``) so the package is self-contained;
  * modern Matplotlib/NumPy compatibility fixes (undefined ``str_wrap`` ->
    ``str_wrap_chem``; deprecated ``plt.cm.get_cmap`` -> ``mpl.colormaps``;
    string-valued ``tick_params(..., 'off')`` -> ``False``; the invalid-escape
    ``'sans\\-serif'`` made an explicit raw string);
  * removed the unused potential-function clusterer and the global-config
    ``main()`` script entry point, and the no-op ``plt.show()`` calls (this is a
    headless, save-to-file pipeline).

The SOM plotting algorithms themselves (component-plane heat maps, U-matrix,
cluster-category overlays, positioning maps) are preserved from the upstream.
"""

import logging  # noqa: F401  (re-exported; upstream relies on it being configured)
...
# Vendored, namespaced SOMPY (Apache-2.0; see som_multimodal/_vendor/sompy/LICENSE).
from som_multimodal._vendor import sompy
from som_multimodal._vendor.sompy.visualization.mapview import View2D
from som_multimodal._vendor.sompy.visualization.umatrix import UMatrixView
from som_multimodal.engine.strings import str_wrap_chem
...
# Default Matplotlib figure sizes (inches) for the standalone show_* helpers.
POSMAP_SIZE = (10, 10)
UMATRIX_SIZE = (10, 10)


def kmeans_clust(som, n_clusters=8, seed=KM_SEED):
    """Cluster a trained SOM's codebook vectors with k-means; return node labels."""
    print("Performing K-means clustering to SOM trained data...")
    cl_labels = clust.KMeans(n_clusters=n_clusters, random_state=seed).fit_predict(
        som.codebook.matrix
    )
...
def show_posmap(som, placement_name_df, raw_name_df, n_clusters, cl_labels,
                ...
                posmap_size=POSMAP_SIZE, fout_posmap="som_posmap.png"):

    fig, ax = plt.subplots(1, 1, figsize=posmap_size)
    ax.set_aspect('equal')
    ...
    if isOutPosmap:
        print("Saving figure of SOM positioning map to {}...".format(fout_posmap))
        fig.savefig(fout_posmap)
...
# We may want to move to python's typing system later
    def show(self, som :sompy.sompy.SOM, cl_labels :list, what='codebook', which_dim='all',
        ...
        self.prepare()
        # Mathtext font to sans-serif. NB: Matplotlib's mathtext parser requires the
        # hyphen to be escaped ("sans\-serif"); we use a raw string so Python keeps the
        # backslash without emitting an invalid-escape warning.
        mpl.rcParams['mathtext.fontset'] = 'custom'
        mpl.rcParams['mathtext.rm'] = r'sans\-serif'
        mpl.rcParams['mathtext.cal'] = r'sans\-serif'
        ...
        while axis_num < len(indtoshow):
            axis_num += 1
            # int(): modern Matplotlib rejects float subplot grid sizes (the upstream
            # arithmetic ``dim / col_sz + 1`` yields a float).
            ax = plt.subplot(int(no_row_in_plot), int(no_col_in_plot), axis_num)
        ...
        plt.tight_layout()
        # (no plt.show(): this is a headless save-to-file pipeline; figures go to savepath)
...
class UMatrixTFP(UMatrixView):
    def show(self, som, placement_name_df, raw_name_df, savepath,
        ...
        fig, ax = plt.subplots(1, 1, figsize=UMATRIX_SIZE)
        ...
        # (no plt.show(): headless rendering; the figure is written to savepath below)
